    ...
    <div class="container-fluid px-4">
        <div class="d-flex justify-content-between align-items-center mb-3 animate-fade-in-up">
            <div>
                ...
                <span class="text-muted small">{{ $order->store->name }}</span>
            </div>
                        ...
                        <ul class="list-group list-group-flush">
                            @foreach ($order->incidents as $incident)
                                <li class="list-group-item stagger-child">
                                    <strong>{{ ucfirst(str_replace('_', ' ', $incident->type)) }}</strong> &mdash;
                                    {{ $incident->description }}
                                    <span class="text-muted d-block small">
                                        {{ $incident->occurred_at->format('d/m/Y H:i') }} &mdash; {{ $incident->user->name }}
                                    </span>
                                </li>
                            @endforeach
                        </ul>
            ...
            <div class="col-md-4">
                <div class="card mb-3 animate-slide-in" style="animation-delay:0.1s;">
                    ...
                    <div class="card-body small">
                        ...
                        <p class="mb-1"><strong>Creado por:</strong> {{ $order->creator->name }}</p>
                        ...
                        @forelse ($order->movements->sortByDesc('created_at') as $movement)
                            <li class="list-group-item stagger-child">
                                ...
                                <div class="text-muted">
                                    {{ $movement->created_at->format('d/m/Y H:i') }} &mdash; {{ $movement->user?->name ?? 'sistema' }}
                                </div>
                            </li>
                        @empty
